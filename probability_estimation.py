...
for row in data:
	...
	for previous_row in current_table:
		...
		if G.IsEdge(int(user), int(previous_user)):
			...
			tau[(previous_user, user)] = (float((val * count) + (timestamp - previous_timestamp)) / (count + 1), count + 1)
			Avnu[(previous_user,user)] += 1
	for p in parents:
		credit[(p, user, curr_action)] = 1.0/len(parents)
	current_table.add((user, action, timestamp))

# Convert tau dict to average
tau = {(v,u):sum(tau[(v,u)]) / float(len(tau[(v,u)])) for v, u in tau}
